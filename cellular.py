...
def mpi_parallel_get_G(solver_data_package, dt):  
  if mpi.is_master_node():
    ...
    solver_data_package['construct|run|exit'] = 'get_G'
...
def mpi_parallel_get_G_loc(solver_data_package, dt):
  if mpi.is_master_node():
    ...
    solver_data_package['construct|run|exit'] = 'get_G_loc'
...
def get_Sigma(solver, Sigma_imp_iw, Gweiss_iw, Us, max_time=5*60, delta=0.1, solver_data_package=None, su2=True, nambu=False):
  if nambu:
...
def cellular_data( nk, niw, ntau, Nx,Ny, beta, skip_lattice=False ):
  ...
  else: "Skipping lattice Green's function..."
...
def mpi_parallel_add_lattice_data(dt, iws, nk, nsites):
  ...
  if not (master_rank is None):
    ...
    AddNumpyData(dt, ['G_IaJb_k_iw'], (niws_per_master, nk,nk, 2*nsites, 2*nsites) ) #optimizing further, we don't even need G0_ij_iw
    #AddNumpyData(dt, ['H0_k'], (nk,nk, 2*nsites,2*nsites))
...
def cellular_set_calc( dt, max_time=5*60, delta=0.1, solver_data_package=None, nambu=True, su2=True ):
  ...
  dt.get_Sigma = lambda: get_Sigma(dt.solver, dt.Sigma_IaJb_imp_iw, dt.Gweiss_IaJb_iw, dt.Us, max_time, delta, solver_data_package, su2, nambu)
...
def cellular_actions(dt, accr):
  ...
  actions = [
    ...
    generic_action( 
      ...
      mixers = [], #[lambda data, it: 0],
      cautionaries = [],#[lambda data, it: 0], allowed_errors = [],            
      printout = lambda data, it: solvers.ctint.dump(data.solver, dt.archive_name, suffix=it),
      short_timings = True
    ),
    generic_action( 
      name = "lattice",
...
def cellular_launcher(Nx, Ny,
  ...
  if mpi.is_master_node():
    ...
    dt = cellular_data( nk, niw, ntau, Nx,Ny, beta, skip_lattice=not parallel_mem_for_lattice )
    ...
    cellular_set_calc( dt, max_time, delta=(0.2 if n==0.5 else 0.0), solver_data_package=solver_data_package, nambu=True, su2=(initial_afm==0.0) )
    if parallel_mem_for_lattice:
    ...
    actions, monitors, convergers = cellular_actions(dt, accr)
